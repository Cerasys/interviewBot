...
export function decryptZoomAppContext(
  context: string,
  secretKey: string = process.env.ZOOM_APP_CLIENT_SECRET as string
) {
  // Decode base64
...
export const createRequestParamString = (params: Record<string, string>) => {
...
export const hmacBase64 = (str: string) =>
  crypto
    .createHmac("sha256", process.env.ZOOM_APP_OAUTH_STATE_SECRET as string)
    .update(str)
    .digest("base64");
