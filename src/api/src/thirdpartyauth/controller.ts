console.log("before axios import");
import axios from "axios";
console.log("after axios import, before http-proxy-middleware import");
import { createProxyMiddleware } from "http-proxy-middleware";
console.log("after http-proxy-middleware import, before zoom-api and zoom-helpers import");
import * as zoomApi from "../util/zoom-api";
import * as zoomHelpers from "../util/zoom-helpers";
console.log("after zoom-api and zoom-helpers import, before store import");
import store from "../util/store";
console.log("after store import, before RequestHandler and HttpError import");
import { RequestHandler } from "express";
import { HttpError } from "../models/http-error";
console.log("after RequestHandler and HttpError import");
...
export const begin: RequestHandler = async (req, res) => {
    console.log(
        "BEGIN 3RD PARTY OAUTH LOGIN HANDLER ==============================================",
        "\n"
    );
    ...
    const params = {
        ...
        response_type: "code",
        client_id: process.env.ZOOM_APP_CLIENT_ID as string,
...
export const zoomAuth: RequestHandler = async (req, res, next) => {
    console.log(
        "ZOOM OAUTH REDIRECT HANDLER ==============================================",
        "\n"
    );
    ...
    req.session.zoomRequestState = undefined;
    ...
    if (!req.query.code) {
        const error = new HttpError("No auth code was provided");
        error.status = 400;
        return next(error);
    }
    ...
    try {
        ...
        req.session.destroy(() => {
            // todo handle session destroy error gracefully
        });
        return next(error);
    }
};
...
export const auth0Auth: RequestHandler = async (req, res, next) => {
    console.log(
        "AUTH0 REDIRECT HANDLER ==============================================",
        "\n"
    );
