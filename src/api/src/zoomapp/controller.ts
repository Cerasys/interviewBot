...
// In-client OAuth 1/2
export const inClientAuthorize: RequestHandler = async (req, res, next) => {
  console.log(
...
// In-Client OAuth 2/2
export const inClientOnAuthorized: RequestHandler = async (req, res, next) => {
  console.log(
...
export const install: RequestHandler = (req, res) => {
  console.log(
    "INSTALL HANDLER ==========================================================",
    "\n"
  );
...
export const auth: RequestHandler = async (req, res, next) => {
  ...
  req.session.destroy(() => { console.log("Unable to destroy session"); });

  // 1a. Check for auth code as parameter on response from zoom.us
  if (!zoomAuthorizationCode) {
    const error = new HttpError("No authorization code was provided");
    error.status = 400;
    return next(error);
  }
